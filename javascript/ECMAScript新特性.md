ECMAScript是一门脚本语言，一般缩写为ES，通常把他看作是JavaScript的标准规范。但实际上JavaScript是ECMAScript的扩展语言，ECMAScript提供了最基本的语法，通俗点来说只是约定了代码的如何编写。

JavaScript实现了标准开发，在语言基础上做了一定的扩展，可以在浏览器环境中操作```DOM```和```BOM```，在```Node```环境可以读写文件等操作。总的来说浏览器环境中的JavaScript等于ECMAScript + DOM + BOM。

## 1. 块级作用域

在```ES2015```之前，```ECMAScript```中只有全局作用域和函数作用域两种类型的作用域，```ES2015```中又新增了一个块级作用域，用一对```{}```包裹起来的范围，例如```if```语句和```for```语句。
...
ES2015以前，块是没有单独的作用域的，这就导致在块中定义的成员外部也可以访问到。例如```if```中去定义的```foo```，在```if```的外面打印是可以正常打印出来的。
...
这一点对于复杂代码是非常不利的，也是不安全的，有了块级作用域之后，可以在代码当中通过```let```声明变量。用法和```var```一样的，只不过通过```let```声明的变量他只能在所声明的这个代码块中被访问到。
